...
- **Webhook 通知**：节点切换和异常时通过 Webhook 推送通知（兼容 Bark / 企业微信 / 钉钉 / 飞书等）
- **Web UI 配置**：所有参数通过 LuCI 界面配置，无需手动编辑文件
  - 策略组下拉选择（自动从 Clash API 拉取）
  - 自动检测 OpenClash Secret 并回填
  - 日志实时刷新、一键清除、一键下载
  - 立即检测按钮（30 秒冷却）
  - 检测任务可排序（上移/下移）
  - 当日切换/失败次数统计（自动每日重置）
...
2. **Clash API 设置**：API 地址、密钥（留空自动从 OpenClash 配置获取）、代理类型、代理地址
3. **Webhook 通知设置**：启用/禁用、Webhook URL（兼容 Bark / 企业微信 / 钉钉 / 飞书）
